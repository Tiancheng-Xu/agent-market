...
export function DisputePage() {
  const { id } = useParams(); const [reviewed, setReviewed] = useState(false);
  return <Localized><><PageHeader eyebrow={`V3 DISPUTE / ${id ?? "CASE"}`} title="One accountable final arbiter" description="V3 has no committee vote. Evidence is reviewed first; only the configured platform arbiter may request a final Sepolia intent, and only a verified TaskResolved receipt may close the case." actions={<Badge tone="amber">EVIDENCE REVIEW</Badge>} /><DemoNotice /><div className="arbitration-flow"><Panel className={reviewed ? "arbitration-step complete" : "arbitration-step active"}><span>01 / EVIDENCE</span><h2>Review the immutable case bundle</h2><p>Confirm order version, artifact hash, acceptance contract, participant wallets and prior transition events.</p><label className="review-check"><input type="checkbox" checked={reviewed} onChange={(event) => setReviewed(event.target.checked)} /> Evidence relationships reviewed</label></Panel><Panel className="arbitration-step"><span>02 / AUTHORITY</span><h2>Configured platform arbiter only</h2><p>The browser cannot grant or transfer this role. Server authorization must match the configured V3 arbiter and Workflow Escrow address.</p><Badge tone="neutral">NO MODEL VOTE</Badge></Panel><Panel className="arbitration-step"><span>03 / READBACK</span><h2>Receipt before state change</h2><p>Timeout, reorg, failed receipt or mismatched TaskResolved event routes the order to manual_review instead of declaring a winner.</p><Badge tone="amber">FAIL CLOSED</Badge></Panel></div><Panel className="ruling-bar"><div><span>Current Gate</span><strong>{reviewed ? "Evidence reviewed" : "Review required"}</strong></div><p>This page does not submit a transaction. The separate platform console owns the authenticated intent and RPC readback.</p><Link className={`button ${reviewed ? "button-primary" : "button-ghost"}`} aria-disabled={!reviewed} to={reviewed ? "/committee" : `/disputes/${id ?? "case"}`}>Open final arbitration console</Link></Panel></></Localized>;
}
