...
export interface BrowserRiskContext {
  ...
  dagRevision: number;
  quoteStatus: "not_issued" | "active" | "confirmed" | "expired" | "superseded" | "manual_review";
  requestId: string;
}
...
export async function readRiskContext(taskId: string): Promise<BrowserRiskContext> {
  ...
  const parsed = exactObject(await payload(await fetch(`/api/tasks/${encodeURIComponent(taskId)}/risk-context`, {
    credentials: "include",
    headers: { accept: "application/json" },
  })), ["taskId", "taskFingerprint", "phase", "dagRevision", "quoteStatus", "requestId"]);
  const quoteStatuses = ["not_issued", "active", "confirmed", "expired", "superseded", "manual_review"];
...
export async function requestRiskQuote(taskId: string, taskFingerprint: string): Promise<BrowserRiskQuote> {
  ...
  const parsed = exactObject(await payload(await fetch(`/api/tasks/${encodeURIComponent(taskId)}/risk-quote`, {
    method: "POST",
    credentials: "include",
    headers: { "content-type": "application/json", accept: "application/json" },
    body: JSON.stringify({ taskFingerprint: fingerprint }),
  })), ["quoteId", "quote", "version", "requestId"]);
...
export async function confirmRiskQuote(
  ...
  const parsed = exactObject(await payload(await fetch(`/api/tasks/${encodeURIComponent(taskId)}/risk-quote/confirm`, {
    method: "POST",
    credentials: "include",
    headers: { "content-type": "application/json", accept: "application/json" },
    body: JSON.stringify({ quoteId, taskFingerprint: fingerprint }),
