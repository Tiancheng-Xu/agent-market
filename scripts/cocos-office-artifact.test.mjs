...
test("Cocos office artifact contains a real responsive runtime build", async () => {
  const [settingsText, configText, sceneImport, index, style, runtimeIndex, officeRuntime] = await Promise.all([
  ...
  assert.match(officeRuntime, /agent-market\.office\.ready\.v1/);
  assert.match(officeRuntime, /agent-market\.office\.snapshot\.v1/);
});
